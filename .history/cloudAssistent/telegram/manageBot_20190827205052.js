...
async function checkCommand(command, id){
    ...
    if (texts[0] == "/createDeployment"){
        ...
        response.map(async (deploy) => await sendMessage(id ,encodeURIComponent(`Nome: ${deploy.metadata.name}   Imagem: ${deploy.spec.template.spec.containers[0].image.split(":")[0]}/n  Versão: ${deploy.spec.template.spec.containers[0].image.split(":")[1]}`)));
    } 
}
