...
async function checkCommand(command, id){
    ...
    if (texts[0] == "/createDeployment"){
        ...
        console.log(response[0].spec.template.spec[0].image) 
        response.map(async (deploy) => await sendMessage(id ,encodeURIComponent(`Nome: ${deploy.metadata.name} Imagem: ${deploy.spec.template.spec[0].image.split[":"][0]} Versão: ${deploy.spec.template.spec[0].image.split[":"][1]}`)));
    } 
}
